...
class FilterEntryBondItemsByDateUseCase {
  List<EntryBondItemModel> execute(String startDate, String endDate, List<EntryBondItemModel> entryBondItems) {
    ...
    return entryBondItems.where((item) {
      ...
      try {
        ...
        log('Error parsing item.date: $entryBondItemDateStr. Error: $e', name: 'FilterEntryBondItemsByDate');
        return false;
      }

      return entryBondItemDate.isAfter(startDateFormat.subtract(const Duration(days: 1))) &&
          entryBondItemDate.isBefore(endDateFormat.add(const Duration(days: 1)));
    }).toList();
  }
}