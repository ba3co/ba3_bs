...
class ProcessEntryBondItemsInIsolateUseCase {
  ...
  Future<List<EntryBondItemModel>> execute(List<EntryBondItems> data) async {
    ...
    final Isolate isolate = await Isolate.spawn(
        _processEntryBondItemsIsolate, [receivePort.sendPort, data]);

    final List<EntryBondItemModel> result =
        await receivePort.first as List<EntryBondItemModel>;
      ...
      for (var currentItem in entry.itemList) {
        ...
        if (key == '27e7c0e6-9734-4319-bafb-483292e87f17') {
          log(
            ...
            name:
                '_processEntryBondItems account id: ${currentItem.account.id} - docId: ${currentItem.docId}',
          );
        }
        ...
        if (mergedMap.containsKey(key)) {
          ...
          double updatedAmount = _accountStatementService.getAmountSign(
                  existingItem.amount!, existingItem.bondItemType!) +
              _accountStatementService.getAmountSign(
                  currentItem.amount, currentItem.bondItemType!);

          BondItemType finalType =
              updatedAmount >= 0 ? BondItemType.debtor : BondItemType.creditor;
