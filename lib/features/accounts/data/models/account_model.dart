...
class AccountModel implements PlutoAdaptable {
  ...
  Map<PlutoColumn, dynamic> toPlutoGridFormat([type]) {
    return {
      ...
      createAutoIdColumn(): '#',
      PlutoColumn(field: '${AppStrings.number.tr} ${AppStrings.account.tr}', type: PlutoColumnType.text(), title: 'رقم الحساب', width: 180): accNumber,
      PlutoColumn(field: '${AppStrings.code.tr} ${AppStrings.account.tr}', type: PlutoColumnType.text(), title: 'رمز الحساب', width: 180): accCode,
      PlutoColumn(field: '${AppStrings.name.tr} ${AppStrings.account.tr}', type: PlutoColumnType.text(), title: 'اسم الحساب'): accName,
      PlutoColumn(field: '${AppStrings.name.tr} ${AppStrings.account.tr} ${AppStrings.al.tr+AppStrings.latin.tr}', type: PlutoColumnType.text(), title: 'الاسم الاتيني'): accLatinName,
      PlutoColumn(field:  '${AppStrings.type.tr} ${AppStrings.account.tr}', type: PlutoColumnType.text(), title: 'نوع الحساب'): AppServiceUtils.getAccountType(accType),
      PlutoColumn(field: AppStrings.fatherAccount.tr, type: PlutoColumnType.text(), title: 'حساب الاب'): accParentName,
      PlutoColumn(field: AppStrings.children.tr, type: PlutoColumnType.text(), title: 'الاولاد'): read<AccountsController>().getAccountChildren(id).join(' , '),
      PlutoColumn(field:AppStrings.customers.tr, type: PlutoColumnType.text(), title: 'الزبائن'): accCustomer?.toList().join(' , '),
    };
  }
}
