  ...
  BillModel _createPlaceholderBill(BillModel referenceBill) {
    return BillModel(
      ...
      billDetails: BillDetails(
        billPayType:
            referenceBill.billTypeModel.latinShortName == 'Sales Delivery'
...
/*    log(' updatedBill.from $from');
  ...
  Future<void> jumpForwardByTen() async => await _navigateToBill(
        currentBill.billDetails.billNumber! + 10,
        NavigationDirection.next,
      );
  ...
  Future<void> head() async => await _navigateToBill(
        1,
        NavigationDirection.next,
      );
  ...
  bool _validateAndHandleBillNumber(
    int billNumber,
  ) {
  ...
  Future<void> _fetchAndNavigateToBill(
    ...
    result.fold(
      ...
      (fetchedBills) => _handleFetchSuccess(
        fetchedBills,
        billNumber,
      ),
    );
