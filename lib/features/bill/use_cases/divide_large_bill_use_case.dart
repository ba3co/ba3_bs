...
class DivideLargeBillUseCase {
  ...
  List<BillModel> execute(BillModel bill,
      {int maxItemsPerBill = AppConstants.maxItemsPerBill}) {
    ...
    if (allItems.length <= maxItemsPerBill) {
      return [
        bill
      ]; // No need to split, return original bill as a single-item list
    }

    // 🔹 Split items into chunks based on `maxItemsPerBill`
    final List<List<BillItem>> itemChunks =
        _splitItemsIntoChunks(allItems, maxItemsPerBill);

    for (int i = 0; i < itemChunks.length; i++) {
      final newBill = bill.copyWith(
        billId:
            "${bill.billId}_part${i + 1}", // Unique identifier for split bills
        billDetails: bill.billDetails
            .copyWith(billNumber: bill.billDetails.billNumber! + i),
  ...
  List<List<BillItem>> _splitItemsIntoChunks(
      List<BillItem> items, int maxItemsPerBill) {
