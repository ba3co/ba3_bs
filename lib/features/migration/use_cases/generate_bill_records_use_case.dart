...
class GenerateBillRecordsUseCase {
  Future<List<InvoiceRecordModel>> execute(
      List<MaterialModel> materials) async {
    final ReceivePort receivePort = ReceivePort();

    // Convert materials to a JSON-serializable format before sending to the isolate
    final List<Map<String, dynamic>> materialsJson =
    ...
    final List<Map<String, dynamic>> billRecordsJson =
        await receivePort.first as List<Map<String, dynamic>>;

    // Convert JSON results back to `InvoiceRecordModel`
    List<InvoiceRecordModel> billRecords = billRecordsJson
        .map((recordJson) => InvoiceRecordModel.fromJson(recordJson))
        .toList();
  ...
  static void _generateBillRecordsIsolate(List<dynamic> args) {
    ...
    final List<MaterialModel> materials =
        materialsJson.map((m) => MaterialModel.fromJson(m)).toList();
    ...
    final List<Map<String, dynamic>> resultJson =
        result.map((record) => record.toJson()).toList();

    sendPort.send(resultJson);
  }

  static List<InvoiceRecordModel> _processBillRecords(
      List<MaterialModel> materials) {
    final List<InvoiceRecordModel> invoiceRecords =
        materials.expand((material) => _processMaterial(material)).toList();

    return invoiceRecords;
  }

  static List<InvoiceRecordModel> _processMaterial(
      MaterialModel materialModel) {
    List<InvoiceRecordModel> invoiceRecords = [];

    List<String> soldSerials = materialModel.serialNumbers?.entries
    ...
    if (soldSerials.isNotEmpty) {
      for (String serial in soldSerials) {
        ...
        invoiceRecords
            .add(InvoiceRecordModel.fromJsonPluto(materialModel.id!, row));
      }
    } else {
  ...
  static Map<String, dynamic> _rowToJson(
      MaterialModel material, String? specificSerial) {
    return {
      AppConstants.invRecProduct: material.matName,
      AppConstants.invRecQuantity: material.matQuantity,
      AppConstants.invRecTotal:
          (material.matQuantity ?? 0) * (material.calcMinPrice ?? 0),
      AppConstants.invRecProductSoldSerial: specificSerial,
      AppConstants.invRecProductSerialNumbers:
          material.serialNumbers?.keys.toList(),
    };
  }
}
