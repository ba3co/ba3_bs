...
class CompoundFireStoreTestService extends ICompoundDatabaseService<Map<String, dynamic>> {
  final _firestore = FirebaseFirestore.instanceFor(app: Firebase.app(), databaseId: 'test');
...
// Helper method to apply the date filter to the query
  ...
  Future<Map<String, dynamic>> fetchById({
    ...
    final docSnapshot = await _firestore.collection(rootCollectionPath).doc(rootDocumentId).collection(subCollectionPath).doc(subDocumentId).get();
  ...
  Future<List<Map<String, dynamic>>> saveAll({
    ...
    for (final item in items) {
      ...
      final docId =
          item.putIfAbsent('docId', () => _firestore.collection(rootCollectionPath).doc(rootDocumentId).collection(subCollectionPath).doc().id);
