...
class UploaderStorageQueryableRepo<T> extends RemoteDataSourceRepository<T> {
  ...
  UploaderStorageQueryableRepo(this._imageLoaderUploaderDataSource)
      : super(_imageLoaderUploaderDataSource);

  Future<Either<Failure, String>> uploadImage(String imagePath) async {
    try {
      final savedItems = await _imageLoaderUploaderDataSource.uploadImage(
          imagePath: imagePath);
  ...
  Future<Either<Failure, String?>> fetchImage(
  ...
  Future<Either<Failure, List<T>>> fetchWhere(
    ...
    try {
      final savedItems = await _imageLoaderUploaderDataSource.fetchWhere(
