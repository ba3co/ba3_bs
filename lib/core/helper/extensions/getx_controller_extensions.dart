...
void lazyPut<T>(T controller, {String? tag, bool fenix = true}) => Get.lazyPut<T>(() => controller, tag: tag, fenix: fenix);
...
T putIfAbsent<T extends GetxController>(T controllerBuilder, {String? tag, bool permanent = false}) {
  if (!isRegistered<T>()) {
...
T lazyPutIfAbsent<T extends GetxController>(T controllerBuilder, {String? tag, bool fenix = true}) {
  if (!isRegistered<T>()) {
